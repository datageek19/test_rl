...
SELECT
    ...
    ARRAY_SIZE(o.FHIR_IDENTIFIER) AS ORGANIZATION_IDENTIFIER_COUNT,
    (i.INDEX + 1) AS ORGANIZATION_SEQUENCE,
    c.VALUE:code::STRING AS ORGANIZATION_IDENTIFIER_TYPE_CD,
    c.VALUE:display::STRING AS ORGANIZATION_IDENTIFIER_TYPE_DISPLAY,
    c.VALUE:system::STRING AS ORGANIZATION_IDENTIFIER_TYPE_CODE_SYSTEM,
    i.VALUE:value::STRING AS ORGANIZATION_IDENTIFIER_TYPE_VALUE,
    o.FHIR_NAME AS ADMITHOSPITAL
FROM
    ECT_PRD_ECDHMOD_DB.FOUNDATION.ORGANIZATION_ADT o,
    LATERAL FLATTEN(INPUT => o.FHIR_IDENTIFIER) i,
    LATERAL FLATTEN(INPUT => i.VALUE:type.coding) c -- Flatten coding array within each FHIR_IDENTIFIER
WHERE
    (c.VALUE:code::STRING ILIKE '%Medicaid%' OR
     c.VALUE:display::STRING ILIKE '%Medicaid%')
limit 100
