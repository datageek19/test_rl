...
class AlertProcessor:
    ...
    def group_alerts_by_relationships(self) -> List[Dict]:
        ...
        Note: Only call this with alerts that have graph_service set.
        Unmapped alerts should go directly to cluster-based grouping.
        """
        ...
        for i, group in enumerate(self.consolidated_groups):
            ...
            for alert in group['alerts']:
                ...
                # Propagate impacted_services from group to each alert for catalog tracking
                alert['impacted_services'] = group.get('impacted_services', [])
                alert['primary_service'] = group.get('primary_service', '')
    ...
    def _calculate_group_metrics(self, group: Dict) -> Dict:
        ...
        return metrics
    # +++++++++++++++++++++++++++++++++++++++++++++++++
    def engineer_features(self, target_pca_components: int = None, alert_features_only: bool = False) -> Tuple[np.ndarray, StandardScaler, PCA, List[str]]:
        """
        Extract features from alerts for clustering.
        
        Args:
            target_pca_components: Number of PCA components to use
            alert_features_only: If True, only use alert metadata features (no graph features).
                                 Use this for unmapped/unmatched alerts that go directly to clustering.
        """
        self.alerts_df = pd.DataFrame(self.enriched_alerts)
        
        mapped_count = sum(1 for alert in self.enriched_alerts if alert.get('graph_service'))
        if alert_features_only:
            print(f"    Engineering ALERT-ONLY features: {len(self.enriched_alerts)} alerts (skipping graph features)")
        else:
            print(f"    Engineering features: {len(self.enriched_alerts)} alerts ({mapped_count} mapped)")
        ...
        for row in self.alerts_df.itertuples():
            ...
            # Graph features - only include if not alert_features_only mode
            if not alert_features_only:
                graph_service = getattr(row, 'graph_service', None)
                if graph_service and graph_service in self._service_features_cache:
                    cached = self._service_features_cache[graph_service]
                    feature_dict['pagerank'] = cached.get('pagerank', 0)
                    feature_dict['degree'] = cached.get('degree_total', 0)
                    feature_dict['in_degree'] = cached.get('in_degree', 0)
                    feature_dict['out_degree'] = cached.get('out_degree', 0)
                    feature_dict['betweenness'] = cached.get('betweenness', 0)
                    feature_dict['num_upstream'] = cached.get('num_upstream', 0)
                    feature_dict['num_downstream'] = cached.get('num_downstream', 0)
                else:
                    # Unmapped alerts get zero graph features
                    feature_dict['pagerank'] = 0
                    feature_dict['degree'] = 0
                    feature_dict['in_degree'] = 0
                    feature_dict['out_degree'] = 0
                    feature_dict['betweenness'] = 0
                    feature_dict['num_upstream'] = 0
                    feature_dict['num_downstream'] = 0
            
            # Alert metadata features (always included)
            feature_dict['severity'] = self._encode_severity(getattr(row, 'severity', ''))
            feature_dict['category'] = self._encode_alert_category(getattr(row, 'alert_category', ''))
            feature_dict['subcategory'] = self._encode_alert_subcategory(getattr(row, 'alert_subcategory', ''))
            
            # Alert type flags (simple keyword matching)
            alert_name = str(getattr(row, 'alert_name', '')).lower()
            feature_dict['is_error'] = 1 if 'error' in alert_name else 0
            feature_dict['is_resource'] = 1 if any(x in alert_name for x in ['cpu', 'memory', 'resource']) else 0
            feature_dict['is_network'] = 1 if 'network' in alert_name else 0
        ...
        print(f"    Features: {self.feature_matrix.shape[1]} dimensions")
    ...
    def _generate_cluster_name(self, cluster_alerts: List[Dict]) -> str:
        ...
        top_service_alerts = []
        for a in cluster_alerts:
            alert_svc = a.get('graph_service') or a.get('service_name') or ''
            if alert_svc == top_service and a.get('alert_name'):
                top_service_alerts.append(a.get('alert_name', ''))
