...
class AlertWorkflowScheduler:
    ...
    def run_workflow(self):
        ...
        try:
                ...
                for parquet_file in parquet_files:
                    try:
                        ...
                        if 'platform' in df.columns:
                            platform_series = df['platform'].astype(str).str.lower()
                            file_aks_count = int((platform_series == 'aks').sum())
                            file_gke_count = int((platform_series == 'gke').sum())
                            logger.info(
                                "File counts - total: %d, aks: %d, gke: %d",
                                file_total,
                                file_aks_count,
                                file_gke_count
                            )
                            df = df[platform_series.isin(['aks', 'gke'])]
                        else:
            ...
            self.catalog_manager.set_graph_context(service_graph, service_features_cache)
            ...
            if len(unmatched_alerts) == 0:
                ...
                new_clusters_created = self.catalog_manager.update_catalog_with_clusters(all_deduplicated)
                ...
                if expected_pca != actual_pca:
                    ...
                    new_clusters_created = self.catalog_manager.update_catalog_with_clusters(unmatched_deduplicated)
            
            # Enforce cluster limits - merge similar clusters and cap catalog size
            print(" Enforcing cluster catalog limits...")
            self.catalog_manager.merge_similar_clusters()
            self.catalog_manager.enforce_cluster_limit()
            
            print(" Saving cluster catalog...")
            self.catalog_manager.save_catalog()
            # Disabled for local debugging - only read from remote, no writes
            ...
            # Track alerts that were newly clustered (not matched to catalog)
            for alert_id in deduplicated_ids:
                if alert_id not in matched_ids:
...
def main():
    ...
    parser.add_argument('--demo', action='store_true', help='Run demo sample transformation and dump to demo.json')

    args = parser.parse_args()

    if args.demo:
        from demo_transform import run_demo_sample_transform
        print("\n[Demo] Running alert processor demo sample transformation...")
        run_demo_sample_transform(Config.GRAPH_JSON_PATH, sample_size=8, output_path="demo.json")
        return
