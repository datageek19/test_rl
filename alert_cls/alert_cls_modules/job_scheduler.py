    ...
    def run_workflow(self):
        ...
        try:
            ...
            parquet_files = sorted(
                glob.glob(os.path.join(alerts_dir, '*.parquet'))
            )
            alerts = []
            ...
            if parquet_files:
                ...
                print(f"\n No parquet files found in: {alerts_dir}")
            ...
            matched_alerts, unmatched_alerts, all_match_types = self.catalog_manager.match_alerts_to_catalog(enriched_alerts)
            ...
            if len(unmatched_alerts) == 0:
                ...
                # bug fix: alerts in consolidated groups will be used for clustering subsequently
                ...
                # ++++++++++++++++++
                # bug fix: unmatched alerts in consolidated groups will be used for clustering subsequently
            ...
            duplicate_count = sum(1 for a in enriched_alerts if a.get('is_duplicate'))
            outliers_removed = len(alert_processor.outlier_indices) if hasattr(alert_processor, 'outlier_indices') else 0
            alerts_after_outlier_removal = len(enriched_alerts) - outliers_removed
            ...
            clustering_stats = {
                ...
                'total_alerts_with_duplicates': len(enriched_alerts),
            ...
            duplicate_alerts = [a for a in enriched_alerts if a.get('is_duplicate') == True]
