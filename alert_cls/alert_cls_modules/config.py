...
# Ensure we load from the same directory as this config file
config_dir = Path(__file__).parent
env_path = config_dir / '.env'
load_dotenv(env_path)

class Config:
    WORKFLOW_INTERVAL_MINUTES = int(os.getenv("WORKFLOW_INTERVAL_MINUTES") or "5")
    ...
    CLUSTER_ACTIVE_HOURS_THRESHOLD = int(os.getenv("CLUSTER_ACTIVE_HOURS_THRESHOLD"))
