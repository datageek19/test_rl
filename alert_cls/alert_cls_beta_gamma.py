...
class ComprehensiveAlertConsolidator:
    ...
    def _enrich_alert_with_graph_info(self, alert):
        ...
        for svc_name, svc_info in self.service_to_graph.items():
            ...
            # needed for both namespace AND cluster for fallback mapping
        ...
        if matched_services:
            ...
            # sanity check for ambiguous matches (multiple services with same top score)
            best_match = matched_services[0]
            top_score = best_match[2]
            
            # sanity check for how many services have the same top score
    ...
    def _deduplicate_within_groups(self):
        ...
        This uses service relationships and is more accurate than post-clustering dedup.
        """
        self.deduplicated_alerts = []
        self.duplicate_groups = []
        total_duplicates = 0
    ...
    def _find_optimal_k_subset(self, feature_subset, max_k=10):
        """Find optimal number of clusters for a subset of features"""
        ...
        for k in K_range:
            kmeans = KMeans(n_clusters=k, random_state=42, n_init=10)
            labels = kmeans.fit_predict(feature_subset)
            try:
                score = silhouette_score(feature_subset, labels)
                silhouette_scores.append(score)
            except:
                silhouette_scores.append(0)
        ...
        for i, alert in enumerate(self.enriched_alerts):
            ...
            if i in self.outlier_indices:
                ...
                if self._idx_mapping and i in reverse_mapping:
                    ...
                    alert['cluster_id'] = int(self.alerts_df.at[new_idx, 'cluster_id'])
                    alert['clustering_method'] = self.alerts_df.at[new_idx, 'clustering_method']
                elif not self._idx_mapping:
                    # No outlier removal, direct mapping
                    alert['cluster_id'] = int(self.alerts_df.at[i, 'cluster_id'])
                    alert['clustering_method'] = self.alerts_df.at[i, 'clustering_method']
                else:
    ...
    # ========================================================================
    # PHASE 6: (OLD DEDUPLICATION - NOW DONE IN PHASE 4.5)
    # ========================================================================
    # Note: Deduplication now happens after graph grouping (Phase 4.5)
    ...
    def _generate_cluster_description(self, cluster_alerts, cluster_id):
        ...
        # Use ONLY the top root cause service's downstream count to avoid double-counting
        # (taking union of multiple services' downstream inflates the count artificially)
        affected_services = set()
        affected_debug = {}
        
        # Sanity check: if a service impacts >80% of graph, it's likely a data quality issue
        # Cap at a reasonable percentage of total graph size
        total_graph_size = len(self.service_graph.nodes()) if self.service_graph else 1000
        max_reasonable_impact = int(total_graph_size * 0.10)  # Cap at 10% of graph
        
        if root_cause_services_preliminary:
            # Take ONLY the #1 root cause service (highest downstream impact)
            ...
            # Apply sanity cap for unrealistic downstream counts
            if len(downstream) > max_reasonable_impact:
                # Service impacts too many services - likely root/shared service
                # Use more conservative estimate from next service or cap
                if len(root_cause_services_preliminary) > 1:
                    # Try second root cause if available
                    second_rc = root_cause_services_preliminary[1]
                    if second_rc['downstream_count'] <= max_reasonable_impact:
                        service = second_rc['service']
                        downstream = second_rc['downstream_services']
                        affected_debug[f"{service} (2nd choice)"] = len(downstream)
                    else:
                        # Both too high, cap at reasonable limit
                        downstream = set(list(downstream)[:max_reasonable_impact])
                        affected_debug[f"{service} (capped)"] = max_reasonable_impact
                else:
                    # Only one service, cap it
                    downstream = set(list(downstream)[:max_reasonable_impact])
                    affected_debug[f"{service} (capped)"] = max_reasonable_impact
            else:
            ...
            if services:
                ...
                if primary_service and self.service_graph.has_node(primary_service):
                    ...
                    # Apply same cap for primary service
                    if len(downstream) > max_reasonable_impact:
                        downstream = set(list(downstream)[:max_reasonable_impact])
                        affected_debug[f"{primary_service} (capped)"] = max_reasonable_impact
                    else:
        ...
        # For debugging, show other potential root causes beyond top 3
        if len(root_cause_services_preliminary) > 3:
            for rc in root_cause_services_preliminary[3:5]:  # Show next 2
                service = rc['service']
                count = rc['downstream_count']
                root_cause_debug.append(f"{service}: {count} downstream (not in top 3)")
        ...
        for service in set(services):
            if service and service not in root_cause_services_set:
                if service and self.service_graph.has_node(service):
                    downstream = self._get_transitive_downstream(service, max_depth=2)
                    downstream_count = len(downstream)
                    if len(root_cause_debug) < 10:  # Limit debug output
                        root_cause_debug.append(f"{service}: {downstream_count} downstream (low impact)")
...
if __name__ == "__main__":
    ...
    consolidator = ComprehensiveAlertConsolidator(
        alerts_csv_path='C:/Users/jshay21/alert_classification/data_classification_beta_prime/saafe_alerts.csv',  
        graph_json_path='C:/Users/jshay21/alert_classification/data_classification_beta_prime/service_graph_data.json',
        output_dir='temp'
    )
